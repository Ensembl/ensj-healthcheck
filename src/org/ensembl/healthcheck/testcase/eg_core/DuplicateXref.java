...
public class DuplicateXref extends AbstractEgCoreTestCase {
	...
	private final static String DUPLICATE_OBJ_XREF = "select count(*) from (select count(*) from xref x join object_xref ox using (xref_id) group by ox.ensembl_id, ox.ensembl_object_type,ox.analysis_id,x.dbprimary_acc,x.external_db_id,x.info_type,x.info_text having count(*)>1) cc";
