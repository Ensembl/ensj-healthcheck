...
public class CircularAwareFeatureCoords extends FeatureCoords {
	...
	public final static String START_END_SQL = "SELECT COUNT(*) FROM $tableName$ "
			+ "WHERE seq_region_start > seq_region_end and "
			+ "seq_region_id not in "
			+ "(select seq_region_id from seq_region_attrib "
			+ "join attrib_type using (attrib_type_id) "
			+ "where code = 'circular_seq' and value=1)";

	@Override
	protected boolean checkStartEnd(String tableName, Connection con) {
		String sql = TemplateBuilder.template(START_END_SQL, "tableName",
				tableName);
		boolean result = true;
		// ------------------------
