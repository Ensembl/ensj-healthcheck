...
/**
 * Compare the gene names in the current database with those from the equivalent
...
public class ComparePreviousVersionProjectedGeneNames extends SingleDatabaseTestCase {
	...
	public ComparePreviousVersionProjectedGeneNames() {
		...
		setDescription(
				"Compare gene names in the current database with those from the equivalent database on the secondary server.");
	...
	public boolean run(DatabaseRegistryEntry dbre) {
		...
		// current
		float displayXrefCount = new Integer(
				DBUtils.getRowCount(currentCon, "SELECT COUNT(1) FROM gene WHERE display_xref_id IS NOT NULL"));
		float displayXrefPreviousCount = new Integer(
				DBUtils.getRowCount(previousCon, "SELECT COUNT(1) FROM gene WHERE display_xref_id IS NOT NULL"));
		...
		if (!nochange) {
			...
			if (missingIds > 0 && percentage > 5) {
				ReportManager.problem(this, currentCon,
						missingIds + "(" + percentage + "%) genes lack projected names in the current database ");
				result = false;
			}
			...
			if (accessionsChanged > 50 && percentage > 5) {
				ReportManager.problem(this, currentCon, accessionsChanged + "(" + percentage
						+ "%) display xref primary accessions changed for the same source ");
				result = false;
			}
