...
public class Meta extends SingleDatabaseTestCase {
	...
	public Meta() {
		...
		addToGroup("release");
		setDescription(
			"Check that the meta table exists, has data, the entries correspond to the " + "database name, and that the values in assembly.type match what's in the meta table");
	}
	...
	public boolean run(DatabaseRegistryEntry dbre) {
		...
		if (metaTableAssemblyVersion == null || metaTableAssemblyDefault == null || metaTableAssemblyPrefix == null || dbNameAssemblyVersion == null) {
			...
			if (!metaTableAssemblyVersion.equalsIgnoreCase(dbNameAssemblyVersion)) {
				...
				ReportManager.problem(
					this,
					con,
					"Database name assembly version (" + dbNameAssemblyVersion + ") does not match meta table assembly version (" + metaTableAssemblyVersion + ")");
			} else {
		...
		Pattern assemblyMappingPattern = Pattern.compile("^([a-zA-Z0-9.]+)(:[a-zA-Z0-9.]+)?\\|([a-zA-Z0-9.]+)(:[a-zA-Z0-9.]+)?(\\|([a-zA-Z0-9.]+)(:[a-zA-Z0-9.]+)?)?$");
		...
		if (dbTaxonID.equals(Species.getTaxonomyID(species))) {
			...
			ReportManager.problem(
				this,
				con,
				"Taxonomy ID " + dbTaxonID + " in database is not correct - should be " + Species.getTaxonomyID(species) + " for " + species.toString());
		}
		// -------------------------------------------
