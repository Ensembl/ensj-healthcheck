...
public class MappingSession extends SingleDatabaseTestCase {
	...
	private boolean checkNoNullStrings(Connection con) {
		...
		if (rows > 0) {
			ReportManager.problem(this, con, rows + " rows in stable_id_event.new_stable_id contains \"NULL\" string instead of NULL value.");
			result = false;
		}
		...
		if (rows > 0) {
			ReportManager.problem(this, con, rows + " rows in stable_id_event.old_stable_id contains \"NULL\" string instead of NULL value.");
			result = false;
		}
