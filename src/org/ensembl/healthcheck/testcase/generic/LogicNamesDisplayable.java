...
public class LogicNamesDisplayable extends SingleDatabaseTestCase {
	...
	private String[] featureTables =
		{ "gene", "prediction_transcript", "dna_align_feature", "marker_feature", "protein_feature", "qtl_feature", "repeat_feature", "simple_feature", "protein_align_feature" };
	...
	private boolean checkProteinFeatureAnalysis(Connection con) throws SQLException {
		...
		ResultSet rs =
			stmt.executeQuery(
				"SELECT a.analysis_id, a.gff_feature, a.gff_source " + "FROM analysis a, protein_feature pf " + "WHERE a.analysis_id = pf.analysis_id " + "GROUP BY a.analysis_id");
			...
			if (gffSource.equals("PRINTS") || gffSource.equals("PFAM") || gffSource.equals("PROSITE") || gffSource.equals("PROFILE")) {
				...
				if (!gffFeat.equals("DOMAIN")) {
					ReportManager.problem(
						this,
						con,
						"protein_feature" + " analysis with analysis_id = " + analysisId + " and gffSource = " + gffSource + " must have gffSource eq 'DOMAIN'");
					noProblems = false;
				}
				...
				if (gffFeat.equals("DOMAIN")) {
					ReportManager.problem(
						this,
						con,
						"protein_feature" + " analysis with analysis_id = " + analysisId + " and gffSource = " + gffSource + " must have gffSource ne 'DOMAIN'");
					noProblems = false;
				}
			} else if (!gffSource.equals("SUPERFAMILY")) {
				/* problem... not a known gffsource */
				ReportManager.problem(this, con, "protein_feature" + " analysis with analysis_id = " + analysisId + " has unknown gff_source = '" + gffSource + "'");
				noProblems = false;
			}
		} // while rs
