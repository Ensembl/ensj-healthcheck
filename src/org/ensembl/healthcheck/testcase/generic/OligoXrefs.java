 ...
 * 
 * Assumptions: oligo xrefs and transcripts are both in the default chromosome coordinate system.
 * 
...
public class OligoXrefs extends SingleDatabaseTestCase {
	 ...
	 * 
	 * Get a list of chromosomes, then check the number of Oligo xrefs associated with each one. Fail is any chromosome has 0 oligo
	 * xrefs.
	 * 
	 * @param dbre
	 *          The database to use.
	...
	public boolean run(DatabaseRegistryEntry dbre) {
		...
		try {
			...
			ResultSet rs = con.createStatement().executeQuery(
					"SELECT seq_region_id, s.name FROM seq_region s, coord_system c WHERE c.coord_system_id=s.coord_system_id AND c.name='chromosome' and attrib='default_version '");
			while (rs.next())
			...
			rs = con
					.createStatement()
					.executeQuery(
							"select seq_region_id, count(*) as count  from transcript t, object_xref ox, xref x where t.transcript_id=ox.ensembl_id and ensembl_object_type='Transcript' and ox.xref_id=x.xref_id and x.external_db_id in ("
									+ inList + ") GROUP BY seq_region_id");
			while (rs.next())
