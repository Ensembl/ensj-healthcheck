...
public class Karyotype extends SingleDatabaseTestCase {
	...
	public boolean run(DatabaseRegistryEntry dbre) {
		...
		Set<String> karyotypeSpecies = CollectionUtils.createLinkedHashSet(
				DatabaseRegistryEntry.DROSOPHILA_MELANOGASTER, DatabaseRegistryEntry.HOMO_SAPIENS,
				DatabaseRegistryEntry.MUS_MUSCULUS, DatabaseRegistryEntry.RATTUS_NORVEGICUS);
	...
	protected boolean checkKaryotype(DatabaseRegistryEntry dbre) {
		...
		String[] patches = DBUtils.getColumnValues(con,
				"SELECT sr.name FROM seq_region sr, assembly_exception ae " + "WHERE sr.seq_region_id=ae.seq_region_id "
		...
		int count = 0;
		try {
			PreparedStatement stmt = con.prepareStatement("SELECT sr.name, MAX(kar.seq_region_end), sr.length "
