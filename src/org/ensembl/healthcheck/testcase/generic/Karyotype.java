...
public class Karyotype extends SingleDatabaseTestCase {
	...
	public boolean run(DatabaseRegistryEntry dbre) {
        ...
        protected boolean karyotypeExists(DatabaseRegistryEntry dbre) {
                ...
                String sqlMT = "SELECT count(*) FROM seq_region_attrib sa, attrib_type at, seq_region s WHERE s.seq_region_id = sa.seq_region_id AND at.attrib_type_id = sa.attrib_type_id AND code = 'karyotype_rank' AND s.name IN ('MT','Mito')";
                ...
                if (karyotype > 0) {
                        ...
                        if (mt == 0 && dbre.getType() != DatabaseType.SANGER_VEGA) {
                                result = false;
                                ReportManager.problem(this, con, "Species has chromosomes but neither MT nor Mito");
                        }
                } 
        return result;
        }
