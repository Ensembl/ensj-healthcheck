...
/**
 * Check that all top-level seq regions have some SNP density features, and 
...
public class DensityFeatures extends SingleDatabaseTestCase {
    ...
    public DensityFeatures() {
        ...
        addToGroup("release");
        setDescription("Check that all top-level seq regions have some SNP density features, and that the values agree between the density_feature and seq_region attrib tables");
	...
	// check if this species has snpDensity features
	int rows = getRowCount(con, "SELECT COUNT(*) FROM analysis WHERE logic_name='snpDensity'");
	if (rows == 0) {
	    logger.info(DBUtils.getShortDatabaseName(con) + " has no snpDensity analysis type, skipping checks for SNP density features");
	    return true;
	}
	...
	try {
	    ...
	    // check each of these (up to a limit) to see how many density features there are
		...
		if (rows == 0) {
		    ...
		    ReportManager.correct(this, con, seqRegionName + " has " + rows + " density features");
		}
		...
		if (rows > 0) {
		    ...
		    logger.fine("Comparing density_feature.density_value with seq_region_attrib for " + seqRegionName);
 
		    sql = "SELECT SUM(df.density_value) FROM density_type dt, density_feature df, analysis a WHERE dt.density_type_id=df.density_type_id AND dt.analysis_id=a.analysis_id AND a.logic_name='snpDensity' AND seq_region_id=" + seqRegionID;
			...
			sql = "SELECT value FROM seq_region_attrib sra, attrib_type at WHERE sra.attrib_type_id=at.attrib_type_id AND at.code='SNPCount' AND seq_region_id=" + seqRegionID;
			...
			if (sumSRA != null && sumSRA.length() > 0) {
				...
				ReportManager.problem(this, con, "Sum of values from density_feature (" + sumFromDensityFeature + ") doesn't agree with value from seq_region_attrib (" + valueFromSeqRegionAttrib + ") for " + seqRegionName);
				result = false;
				
			    } else {
				
				ReportManager.correct(this, con, "density_feature and seq_region_attrib values agree for seq region " + seqRegionName);
