...
public class ESTStableID extends SingleDatabaseTestCase {
	...
	public boolean run(DatabaseRegistryEntry dbre) {
		...
		while (it.hasNext()) {
			...
			String sql = "SELECT COUNT(*) FROM " + table + "_stable_id si, " + table + " x, analysis a WHERE a.analysis_id=x.analysis_id AND x." + table + "_id=si." + table + "_id AND a.logic_name LIKE '%est%' AND si.stable_id NOT REGEXP '" + regexp + "'";
