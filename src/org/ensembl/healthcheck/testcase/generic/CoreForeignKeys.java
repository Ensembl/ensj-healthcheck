...
public class CoreForeignKeys extends SingleDatabaseTestCase {
    ...
    public boolean run(DatabaseRegistryEntry dbre) {
        ...
        result &= checkForOrphans(con, "regulatory_factor_transcript", "transcript_id", "transcript", "transcript_id", true);

        result &= checkForOrphans(con, "regulatory_factor_transcript", "regulatory_factor_id", "regulatory_factor", "regulatory_factor_id", true);
    ...
    private boolean checkRegulatoryFeatureKeys(Connection con, String type) {
        ...
        int rows = getRowCount(con, "SELECT COUNT(*) FROM regulatory_feature_object rfo LEFT JOIN " + table + " ON rfo.ensembl_object_id=" + table
                + "." + table + "_id WHERE rfo.ensembl_object_type=\'" + type + "\' AND rfo.ensembl_object_id IS NULL");
