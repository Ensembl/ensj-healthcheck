...
public class CheckSynteny extends SingleDatabaseTestCase {
    ...
    public boolean run(DatabaseRegistryEntry dbre) {
        ...
        if (!tableHasRows(con, "synteny_region")) {
            ...
            for (int i = 0; i < method_link_species_set_ids.length; i++) {
                boolean this_result = check_this_synteny(con, method_link_species_set_ids[i]);
                if (!this_result) {
                  result = false;
                }
            }
    ...
    private String[] get_all_method_link_species_set_ids(Connection con) {
        ...
        String[] method_link_ids = getColumnValues(con,
        ...
        for (int i = 0; i < method_link_ids.length; i++) {
            String[] these_method_link_ids = getColumnValues(con,
        ...
        return (String[]) method_link_species_set_ids.toArray(new String[method_link_species_set_ids.size()]);
    }

    private boolean check_this_synteny(Connection con, String method_link_species_set_id) {
        boolean result = true;

        String[] genome_db_ids = getColumnValues(con,
            "SELECT genome_db_id FROM method_link_species_set LEFT JOIN species_set"
            + " USING (species_set_id) WHERE method_link_species_set_id = " + method_link_species_set_id);
        String[] alignment_mlss_id = getColumnValues(con,
            ...
            + method_link_species_set_id + " AND mlss1.species_set_id = mlss2.species_set_id"
            + " AND mlss2.method_link_id = ml.method_link_id AND ml.class like 'GenomicAlignBlock%'");
        for (int i = 0; i < genome_db_ids.length; i++) {
            String genome_db_name = getRowColumnValue(con, "SELECT name FROM genome_db "
                + " WHERE genome_db_id = " + genome_db_ids[i]);
            String[] these_dnafrag_ids = getColumnValues(con,
            ...
            for (int j = 0; j < these_dnafrag_ids.length; j++) {
                ...
                + " AND dnafrag_id = " + these_dnafrag_ids[j]);
                if (count == 0) {
                    int aln_count = 0;
                    String aln_name = "";
                    String aln_mlss_id = "";
                    for (int k = 0; k < alignment_mlss_id.length; k++) {
                        String[] aln_result = getRowValues(con, "SELECT dnafrag.name, count(*) FROM"
                    ...
                    if (aln_count > 1000) {
                        String dnafrag_name = getRowColumnValue(con, "SELECT name FROM dnafrag "
                            + " WHERE dnafrag_id = " + these_dnafrag_ids[j]);
                        String dnafrag_length = getRowColumnValue(con, "SELECT length FROM dnafrag "
                            + " WHERE dnafrag_id = " + these_dnafrag_ids[j]);
                        ReportManager.problem(this, con, aln_count + " alignments to "
                            + genome_db_name + " chr." + dnafrag_name +
                            " and no syntenies for MLSS " + method_link_species_set_id);
                        result = false;
                    }
                }
            }
