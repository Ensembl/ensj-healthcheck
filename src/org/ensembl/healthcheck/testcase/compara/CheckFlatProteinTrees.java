...
public class CheckFlatProteinTrees extends AbstractTemplatedTestCase {
	...
	protected boolean runTest(DatabaseRegistryEntry dbre) {
        ...
        String internalNodeCountSql = "SELECT gtn.root_id, count(*) AS internal_nodes FROM gene_tree_node gtn LEFT JOIN gene_tree_member gtm USING (node_id) WHERE gtm.node_id IS NULL AND gtn.node_id <> gtn.root_id AND gtn.root_id <> 0 GROUP BY gtn.root_id";
        ...
        String flatMemberCountSql = "SELECT gtn.root_id, count(*) AS root_members FROM gene_tree_node gtn JOIN gene_tree_member gtm USING (node_id) WHERE gtn.parent_id = gtn.root_id AND gtn.root_id <> 0 GROUP BY gtn.root_id having root_members > 1";
