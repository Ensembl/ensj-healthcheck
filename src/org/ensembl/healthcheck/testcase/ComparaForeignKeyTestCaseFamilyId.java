...
public class ComparaForeignKeyTestCaseFamilyId extends EnsTestCase {
  
  /**
   * Create an OrphanTestCase that applies to a specific set of databases.
   */
  public ComparaForeignKeyTestCaseFamilyId() {
  ...
  public TestResult run() {
    ...
    while (it.hasNext()) {
      ...
      // 1 test to check family_id used as foreign key
      ...
      if( getRowCount( con, "select count(*) from family" ) > 0 ) {
	    ...
	    ReportManager.problem(this, con, "family_member <-> family TEST NOT COMPLETED, look at the StackTrace if any");
        }
      } else {
