...
public class DuplicateProbes extends AbstractTemplatedTestCase {
  ...
  protected boolean runTest(DatabaseRegistryEntry dbre) {
    ...
    List<String> arraysWithDuplicateProbes = s.queryForDefaultObjectList(
        "select array.name from array join array_chip using (array_id) join probe using (array_chip_id) where array.is_probeset_array = false group by array.name, probe.name having count(distinct probe.probe_id)>1 limit 1;",
        String.class
     );
