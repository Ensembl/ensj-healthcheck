...
public class ConfigurationProcessor<T> {
	...
	/**
	 * @param A method of type java.lang.reflect.Method
	 * @return true, if the method expects a java.util.List of Strings, 
	 *     false otherwise.
	 */
	boolean listOfStringsExpected(Method m) {
		...
		if (t instanceof ParameterizedType) {
			...
			listOfStringsExpected = rawType.equals(java.util.List.class) && actualType.equals(java.lang.String.class);  
	 ...
	 * 
	 * getHost will return host
	 * getDatabase will return database
	 * etc.
	 * 
	 */
	...
	String getVariableRequested(String methodName) {
		...
		throw new IllegalArgumentException("Method " + methodName + " is an unknown type of method!");
	}

	/**
	 * <p>
	 * Creates a map that maps the name of a variable, as it may be requested 
	 * in a get method like configuration.get*someVariable* to all its possible
	 ...
	 * 
	 */
	public Map<String,Set<String>> createVarName2AllAliases(Class<T> configurationClass) {
		...
		for (Method getMethod : getMethods) {
			...
			varName2AllAliases.put(
				getVariableRequested(getMethod), 
	...
	public Map<String,configurationDataType> createVarName2DataType(Class<T> configurationClass) {
	 ...
	 * parameter and returns the names (aliases) by which this parameter 
	 * could be requested for as a set of strings.
	 * </p>
	 * 
	 * <p>
	 ...
	 *     outputDatabase which may be represented in the properties file
	 *     as "outputDatabase" or "output.database". The alternatives can
	 *     be specified as an Option annotation in the configuration 
	 *     interface like this
	 *     
	 *     		@Option(longName="output.databases")
	 *     		List<String> getOutputDatabases();
	 ...
	 * 
	 */
	public Map<String,Set<String>> createParameterAliasesMap(Class<T> configurationClass) {
	...
	public List<Method> getGetMethods(Class<T> configurationClass) {
		...
		for (Method method : methods) {
			...
			boolean isGetMethodForConfiguration
				= methodName.startsWith("get") && !(specialGetMethodNames.contains(methodName));
